...
lora_init(void)
{
    ...
    io_conf.pin_bit_mask = (1ULL << CONFIG_LORA_RST_GPIO);
    gpio_config(&io_conf);
    ...
    {
        .pin_bit_mask = (1ULL << CONFIG_LORA_CS_GPIO),
    };
 ...
 */
void 
lora_send_packet_non_blocking(uint8_t *buf, int size)
{
   /*
    * Transfer data to radio.
    */
...
lora_receive_packet(uint8_t *buf, int size)
{
   ...
   {
      ESP_LOGE(TAG, "lora_receive_packet ((irq & IRQ_RX_DONE_MASK) == 0)");
      return 0;
   }
   if(irq & IRQ_PAYLOAD_CRC_ERROR_MASK) 
   {
      ESP_LOGE(TAG, "lora_receive_packet (irq & IRQ_PAYLOAD_CRC_ERROR_MASK)");
      return 0;
   }
   /*
    * Find packet size.
   ...
   if (__implicit) 
   {
      ...
      ESP_LOGI(TAG, "lora_receive_packet len = lora_read_reg(REG_PAYLOAD_LENGTH) = %d", len);
   }
   else 
   {
      len = lora_read_reg(REG_RX_NB_BYTES);
      ESP_LOGI(TAG, "lora_receive_packet len = lora_read_reg(REG_RX_NB_BYTES) = %d", len);
   }
